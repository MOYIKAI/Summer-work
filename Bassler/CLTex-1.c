...
// by Kevin E Bassler, originally written 7/2021, last modified July 21, 2021
//
// Increments are +/- 1. 
...
int main(int argc, char *argv[]){   // argc and argv used for command line input
  ...
  int *histo;		// dimension pointer to int (needed for NR ivector subroutine)
  ...
  if (argc == 6){		// require argc be equal to number of command line entries
    ...
    fprintf(stderr,"Usage: CLTex.x M N seed W outfile \n");  // correct input syntax
    return 1;
  }
  ...
  for (i=0; i<M; ++i){					// loop over walks
    ...
    for (j=0; j<N; ++j){				// loop over steps in walk
      // Next two lines create a random increment delta of either -1 or +1  
      ddd = 2*ran2(&seed);				// ddd is randomly either 0 or 1
      delta = 2*ddd-1;					// convert ddd to either -1 or 1
...
// next six lines finds min and max extent of non-zero bins in histo
  ...
  outfile = fopen(argv[5],"w");				// get output filename from command line + open output file
  for (bin=minbin; bin<=maxbin; ++bin){		// loop over non-zero bins for output
    ...
    fprintf(outfile,"%ld, %le \n",W*bin,value);		// write normalized value to output file
  }
  fclose(outfile);					// close output file

  free_ivector(histo,-nbins,nbins);			// NR subroutine to free allocated memory 
