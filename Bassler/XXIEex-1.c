...
int main(int argc, char *argv[]){   // argc and argv used for command line input
  ...
  if (argc == 5){		// require argc be equal to number of command line entries
    ...
    fprintf(stderr,"Usage: XXIEex.x NI NE seed tmax \n");  // correct input syntax
...
int MCupdate(int firstnode){
  ...
  if (ListLength[firstnode] == 0){
    secondnode = -1;
  } else {
    j = ListLength[firstnode]*ran2(&seed);
    secondnode = ConList[firstnode][j];
    if (j < (ListLength[firstnode] - 1)){
      l = ConList[firstnode][ListLength[firstnode]-1];
      ConList[firstnode][j] = l;
      ListLoc[firstnode][l] = j;
    }
    --ListLength[firstnode];
    if (firstnode < NI) {
