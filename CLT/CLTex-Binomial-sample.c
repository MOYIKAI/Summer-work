...
// by Kevin E Bassler, originally written 7/2021, last modified August 6, 2021
//
// Increments are +/- 1. 
...
int main(int argc, char *argv[]){   // argc and argv used for command line input
  ...
  long i,j;
  double smean, value, errors, nSIG;
  long minbin, maxbin;
  ...
  for (i=0; i<M; ++i){					// loop over walks
    ...
    for (j=0; j<N; ++j){				// loop over steps in walk
      ...
      iii = 2*ran2(&seed);				// iii is randomly either 0 or 1
      delta = 2*iii-1;					// convert iii to either -1 or 1
...
// next six lines finds min and max extent of non-zero bins in histo
  ...
  for (bin=minbin; bin<=maxbin; ++bin){		// loop over non-zero bins for output
    ...
    smean = ((double) W*bin/N);
    fprintf(outfile,"%le, %le, %le \n",smean,value,errors);		// write normalized value to output file
  }
